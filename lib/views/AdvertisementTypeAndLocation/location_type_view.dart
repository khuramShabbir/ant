import 'package:demo/commons/app_assets.dart';
import 'package:demo/commons/app_text_styles.dart';
import 'package:demo/commons/widgets.dart';
import 'package:demo/components/app_extended_button.dart';
import 'package:demo/components/app_extended_button_rounded.dart';
import 'package:demo/views/Business_Info_Views/business_info_view.dart';
import 'package:flutter/material.dart';

class LocationTypeView extends StatelessWidget {
  const LocationTypeView({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: [
          Expanded(
            child: Container(
              decoration: BoxDecoration(
                  image: DecorationImage(
                      image: AssetImage(AppAssets.mainBG), fit: BoxFit.fill)),
            ),
          ),
          SizedBox(
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 20),
              child: Column(
                children: [
                  AppWidgets.spacer(verticalSpace: 20),
                  Row(
                    children: [
                      AppWidgets.blueBar(),
                      Text(
                        "Display Location",
                        style: AppTextStyles.subTitleStyle,
                      )
                    ],
                  ),
                  AppWidgets.spacer(verticalSpace: 20),
                  AppExtendedButtonFilled(
                    onTap: () {
                      Navigator.push(context,
                          MaterialPageRoute(builder: (BuildContext context) {
                        return const BusinessInfoView();
                      }));
                    },
                    title: "Static",
                  ),
                  Text(
                    "Usually located by a shop window",
                    style: TextStyle(
                        color: Colors.grey.withOpacity(.5), fontSize: 12),
                  ),
                  AppWidgets.spacer(verticalSpace: 5),
                  AppExtendedButtonRounded(
                    onTap: () {},
                    title: "Variable ",
                  ),
                  const Text(
                    "Usually located inside a passenger transport vehicle",
                    style: TextStyle(fontWeight: FontWeight.bold),
                  ),
                  AppWidgets.spacer(verticalSpace: 15),
                ],
              ),
            ),
          )
        ],
      ),
    );
  }
}
